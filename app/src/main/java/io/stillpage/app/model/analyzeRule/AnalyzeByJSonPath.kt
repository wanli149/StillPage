...
class AnalyzeByJSonPath(json: Any) {
    ...
    companion object {
        ...
        fun parse(json: Any): ReadContext {
            // Use ParseContext configured with SUPPRESS_EXCEPTIONS to avoid PathNotFoundException
            return when (json) {
                is ReadContext -> json
    ...
    fun getString(rule: String): String? {
        ...
        if (rules.size == 1) {
            ...
            if (result.isEmpty()) { // st为空，表明无成功替换的内嵌规则
                try {
                    // JsonPath 可能返回 null（SUPPRESS_EXCEPTIONS），使用空安全默认值
                    val ob: Any? = try { ctx.read<Any>(rule) } catch (_: Throwable) { null }
                    result = when (ob) {
                        is List<*> -> ob.joinToString("\n")
                        is String -> ob
                        null -> ""
                        else -> ob.toString()
                    }
