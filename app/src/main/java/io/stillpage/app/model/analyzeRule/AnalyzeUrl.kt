package io.stillpage.app.model.analyzeRule
...
import io.stillpage.app.constant.AppConst.UA_NAME
import io.stillpage.app.constant.AppPattern
import io.stillpage.app.constant.AppPattern.JS_PATTERN
import io.stillpage.app.constant.AppPattern.dataUriRegex
...
import io.stillpage.app.help.ConcurrentRateLimiter
...
import io.stillpage.app.help.glide.GlideHeaders
import io.stillpage.app.help.http.BackstageWebView
import io.stillpage.app.help.http.CookieManager
import io.stillpage.app.help.http.CookieManager.mergeCookies
import io.stillpage.app.help.http.CookieStore
import io.stillpage.app.help.http.RequestMethod
import io.stillpage.app.help.http.StrResponse
import io.stillpage.app.help.http.addHeaders
import io.stillpage.app.help.http.get
import io.stillpage.app.help.http.getProxyClient
import io.stillpage.app.help.http.newCallResponse
import io.stillpage.app.help.http.newCallStrResponse
import io.stillpage.app.help.http.postForm
import io.stillpage.app.help.http.postJson
import io.stillpage.app.help.http.postMultipart
import io.stillpage.app.help.source.getShareScope
import io.stillpage.app.utils.EncoderUtils
import io.stillpage.app.utils.GSON
import io.stillpage.app.utils.GSONStrict
...
import io.stillpage.app.utils.get
import io.stillpage.app.utils.isJson
import io.stillpage.app.utils.isJsonArray
import io.stillpage.app.utils.isJsonObject
import io.stillpage.app.utils.isXml
...
class AnalyzeUrl(
    ...
    private fun setCookie() {
        // 合并不同层级域的Cookie：解析当前主机与父域，叠加保存的Cookie
        val currentHost = try {
            Uri.parse(if (!baseUrl.isNullOrBlank()) baseUrl else url).host
        } catch (_: Throwable) { null }

        val cookiePrimary = CookieStore.getCookie(domain)
        val cookieHost = currentHost?.let { CookieStore.getCookie(it) } ?: ""
        val parentDomain = kotlin.run {
            currentHost?.let {
                val parts = it.split('.')
                if (parts.size >= 2) parts.takeLast(2).joinToString(".") else null
            }
        }
        val cookieParent = parentDomain?.let { CookieStore.getCookie(it) } ?: ""

        var merged: String? = headerMap["Cookie"]
        merged = mergeCookies(cookiePrimary, merged)
        merged = mergeCookies(cookieHost, merged)
        merged = mergeCookies(cookieParent, merged)
        merged?.let { headerMap["Cookie"] = it }
        if (enabledCookieJar) {
    ...
    companion object {
        ...
        fun AnalyzeUrl.getMediaItem(): MediaItem {
            val headers = getMediaRequestHeaders()
            return ExoPlayerHelper.createMediaItem(url, headers)
        }

        /**
         * 提供给播放器的媒体请求头快照
         * - 统一注入 Cookie/UA/Referer/Origin/Accept 等默认值
         * - 返回独立的 Map 副本，避免外部误改内部 headerMap
         */
        fun AnalyzeUrl.getMediaRequestHeaders(): Map<String, String> {
            // 确保Cookie与默认Header齐备
            setCookie()
            // 注入默认的媒体请求头：Referer/Origin/User-Agent
            try {
                // 默认 User-Agent：优先已有，其次系统WebView UA，最后全局UA
                if (headerMap.get(UA_NAME, true).isNullOrBlank()) {
                    val webUa = kotlin.runCatching { WebSettings.getDefaultUserAgent(appCtx) }.getOrNull()
                    headerMap[UA_NAME] = webUa?.takeIf { it.isNotBlank() } ?: AppConfig.userAgent
                }
                // 默认 Referer：优先使用完整的 baseUrl（内容页），否则使用 url 的基础域
                val referer = kotlin.run {
                    val b = baseUrl
                    if (!b.isNullOrBlank()) b else NetworkUtils.getBaseUrl(url) ?: url
                }
                if (headerMap["Referer"].isNullOrBlank() && !referer.isNullOrBlank()) {
                    headerMap["Referer"] = referer
                }
                // 默认 Origin：scheme://host
                val origin = kotlin.run {
                    val src = if (!baseUrl.isNullOrBlank()) baseUrl else url
                    try {
                        val base = NetworkUtils.getBaseUrl(src)
                        base?.let {
                            val idx = it.indexOf("//")
                            if (idx >= 0) {
                                val end = it.indexOf('/', idx + 2)
                                if (end > 0) it.substring(0, end) else it
                            } else it
                        }
                    } catch (_: Throwable) { null }
                }
                if (headerMap["Origin"].isNullOrBlank() && !origin.isNullOrBlank()) {
                    headerMap["Origin"] = origin
                }
                // 当媒体链接为HTTP且Referer为HTTPS时，部分站点要求HTTP Referer，做简洁转换以提升兼容性
                kotlin.runCatching {
                    val mediaScheme = try { Uri.parse(url).scheme?.lowercase() } catch (_: Throwable) { null }
                    val currentReferer = headerMap["Referer"]
                    val ua = headerMap.get(UA_NAME, true) ?: ""
                    if (mediaScheme == "http" && !currentReferer.isNullOrBlank() && currentReferer.startsWith("https:", true)) {
                        if (ua.startsWith("Mozilla", true)) {
                            headerMap["Referer"] = "http" + currentReferer.substring(5)
                        }
                    }
                }
                // 一般 Accept 兜底
                if (headerMap["Accept"].isNullOrBlank()) {
                    headerMap["Accept"] = "*/*"
                }
                // 默认 Accept-Language（与常见站点匹配，提高命中）
                if (headerMap["Accept-Language"].isNullOrBlank()) {
                    headerMap["Accept-Language"] = "zh-CN,zh;q=0.9"
                }
                // 默认 Connection
                if (headerMap["Connection"].isNullOrBlank()) {
                    headerMap["Connection"] = "keep-alive"
                }
            } catch (_: Throwable) {}
            // 返回副本，避免外部修改影响内部状态
            return LinkedHashMap(headerMap)
        }
