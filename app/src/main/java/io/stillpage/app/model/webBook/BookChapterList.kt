package io.stillpage.app.model.webBook
    ...
    private suspend fun analyzeChapterList(
        ...
        if (elements.isNotEmpty()) {
            ...
            elements.forEachIndexed { index, item ->
                ...
                // 短剧特殊处理：如果已经获取到了有效的播放URL，不要覆盖它
                val isDramaSource = bookSource.bookSourceUrl.contains("vod") || 
                                  bookSource.bookSourceName.contains("短剧") ||
                                  baseUrl.contains("vod") ||
                                  baseUrl.contains(".m3u8")
                
                if (bookChapter.url.isEmpty() && !isDramaSource) {
                ...
                } else if (bookChapter.url.isEmpty() && isDramaSource) {
                    // 短剧源但URL为空，保留为空，后续通过getContent获取真实URL
                    Debug.log(
                        bookSource.bookSourceUrl,
                        "⇒短剧目录${index}未获取到url,保留为空，将通过getContent获取"
                    )
                }
