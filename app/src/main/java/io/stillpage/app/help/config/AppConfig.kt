...
object AppConfig : SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    var isEInkMode = appCtx.getPrefString(PreferKey.themeMode, "3") == "3"
    ...
    var themeMode = appCtx.getPrefString(PreferKey.themeMode, "3")
    ...
    // 自动验证码相关配置
    var enableAutoVerificationCode = appCtx.getPrefBoolean(PreferKey.enableAutoVerificationCode, true)
    var enableOnlineOCR = appCtx.getPrefBoolean(PreferKey.enableOnlineOCR, false)
    var ocrServiceUrl = appCtx.getPrefString(PreferKey.ocrServiceUrl, "")
    var autoVerificationCodeTimeout = appCtx.getPrefInt(PreferKey.autoVerificationCodeTimeout, 10)
    var autoSubmitVerificationCode = appCtx.getPrefBoolean(PreferKey.autoSubmitVerificationCode, false)

    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        when (key) {
            PreferKey.themeMode -> {
                themeMode = appCtx.getPrefString(PreferKey.themeMode, "3")
    ...
    var exploreLayout: Int
        get() = appCtx.getPrefInt(PreferKey.exploreLayout, 1) // 默认网格布局
        set(value) {
            appCtx.putPrefInt(PreferKey.exploreLayout, value)
        }

    var useNewExplore: Boolean
        get() = appCtx.getPrefBoolean(PreferKey.useNewExplore, false) // 默认使用原有发现页面
        set(value) {
            appCtx.putPrefBoolean(PreferKey.useNewExplore, value)
        }

    // 发现页类型TTL（分钟）配置，便于动态调优
    var exploreTtlDramaMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlDramaMin, 30)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlDramaMin, value) }

    var exploreTtlMusicMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlMusicMin, 120)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlMusicMin, value) }

    var exploreTtlAudioMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlAudioMin, 180)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlAudioMin, value) }

    var exploreTtlImageMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlImageMin, 180)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlImageMin, value) }

    var exploreTtlTextMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlTextMin, 60)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlTextMin, value) }

    var exploreTtlFileMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlFileMin, 120)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlFileMin, value) }

    var exploreTtlAllMin: Int
        get() = appCtx.getPrefInt(PreferKey.exploreTtlAllMin, 45)
        set(value) { appCtx.putPrefInt(PreferKey.exploreTtlAllMin, value) }

    // 书源TTL映射（格式：url=分钟，多项以逗号或换行分隔）
    var exploreSourceTtl: String?
        get() = appCtx.getPrefString(PreferKey.exploreSourceTtl)
        set(value) {
            if (value.isNullOrEmpty()) {
                appCtx.removePref(PreferKey.exploreSourceTtl)
            } else {
                appCtx.putPrefString(PreferKey.exploreSourceTtl, value)
            }
        }
     ...
     * 是否显示成人内容（发现页与搜索等）
     */
    var enableAdultContent: Boolean
        get() = appCtx.getPrefBoolean(PreferKey.enableAdultContent, false)
        set(value) {
            appCtx.putPrefBoolean(PreferKey.enableAdultContent, value)
        }

    /**
     * 成人内容加权判定阈值（分数达到或超过该值即视为成人内容）
     * 默认值：6
     */
    var adultScoreThreshold: Int
        get() = appCtx.getPrefInt(PreferKey.adultScoreThreshold, 6)
        set(value) {
            appCtx.putPrefInt(PreferKey.adultScoreThreshold, value)
        }
    ...
    val webDavDir get() = appCtx.getPrefString(PreferKey.webDavDir, "stillpage")
    ...
    // 音频播放速度（持久化）
    var audioPlaySpeed: Float
        get() = appCtx.getPrefString(PreferKey.audioPlaySpeed, "1.0")?.toFloatOrNull() ?: 1.0f
        set(value) {
    ...
    var enableHaptics: Boolean
        ...
        set(value) {
            appCtx.putPrefBoolean(PreferKey.enableHaptics, value)
        }

    // 音频进度保存节流窗口（毫秒）
    var saveThrottleMs: Int
        get() = appCtx.getPrefInt(PreferKey.saveThrottleMs, 2000)
        set(value) {
            appCtx.putPrefInt(PreferKey.saveThrottleMs, value)
        }
