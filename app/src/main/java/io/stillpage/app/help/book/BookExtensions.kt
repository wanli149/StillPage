...
package io.stillpage.app.help.book
...
import io.stillpage.app.data.entities.BaseBook
...
val Book.isDrama: Boolean
    get() {
        // 检查书籍名称、分类、简介中是否包含短剧关键词
        val content = "${name} ${kind ?: ""} ${intro ?: ""} ${originName}".lowercase()
        val dramaKeywords = listOf(
            "短剧", "微短剧", "网剧", "迷你剧", "剧集", "连续剧",
            "电视剧", "网络剧", "drama", "series", "episode"
        )
        return dramaKeywords.any { content.contains(it) }
    }
