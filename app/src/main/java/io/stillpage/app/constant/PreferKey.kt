...
object PreferKey {
    ...
    const val exploreLayout = "exploreLayout"
    const val useNewExplore = "useNewExplore"
    const val enableAdultContent = "enableAdultContent"
    // 成人内容加权判定的阈值（分数达到或超过该值视为成人内容）
    const val adultScoreThreshold = "adultScoreThreshold"
    ...
    // 自动验证码相关
    const val enableAutoVerificationCode = "enableAutoVerificationCode"
    const val enableOnlineOCR = "enableOnlineOCR"
    const val ocrServiceUrl = "ocrServiceUrl"
    const val autoVerificationCodeTimeout = "autoVerificationCodeTimeout"
    const val autoSubmitVerificationCode = "autoSubmitVerificationCode"
    ...
    // 探索/发现页 缓存TTL配置（单位：分钟）
    const val exploreTtlDramaMin = "exploreTtlDramaMin"
    const val exploreTtlMusicMin = "exploreTtlMusicMin"
    const val exploreTtlAudioMin = "exploreTtlAudioMin"
    const val exploreTtlImageMin = "exploreTtlImageMin"
    const val exploreTtlTextMin = "exploreTtlTextMin"
    const val exploreTtlFileMin = "exploreTtlFileMin"
    const val exploreTtlAllMin = "exploreTtlAllMin"
    // 书源TTL映射（格式：url=分钟，多项以逗号或换行分隔）
    const val exploreSourceTtl = "exploreSourceTtl"
}
