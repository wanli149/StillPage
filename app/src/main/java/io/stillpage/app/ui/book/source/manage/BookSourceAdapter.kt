...
class BookSourceAdapter(
    ...
    override fun convert(
        ...
        binding.run {
            if (payloads.isEmpty()) {
                ...
                upContentTypeLabel(tvContentType, item)
    ...
    private fun upContentTypeLabel(tv: TextView, part: BookSourcePart) {
        ...
        tv.text = if (isManuallyTagged) {
            "${ContentTypeUi.label(type)}✓"  // 添加勾号表示手动标记
        } else {
        ...
        // 创建带圆角的背景drawable，保持原有的圆角效果
        val backgroundColorRes = if (isManuallyTagged) {
            ContentTypeUi.backgroundRes(type)  // 使用原色
        } else {
            ContentTypeUi.backgroundResLight(type)  // 使用浅色表示智能检测
        }
