...
import io.stillpage.app.data.entities.rule.TocRule
import io.stillpage.app.databinding.ActivityBookSourceEditBinding
...
import io.stillpage.app.ui.widget.dialog.UrlOptionDialog
import io.stillpage.app.ui.widget.dialog.VariableDialog
import io.stillpage.app.ui.widget.keyboard.KeyboardToolPop
import io.stillpage.app.ui.widget.recycler.NoChildScrollLinearLayoutManager
import io.stillpage.app.ui.widget.text.EditEntity
import io.stillpage.app.utils.GSON
import io.stillpage.app.utils.imeHeight
...
//            6 -> reviewEntities
    ...
    private fun upSourceView(bookSource: BookSource?) {
        ...
        bs.let {
            ...
            // 内容类型覆写 Spinner 显示
            val overrideIndex = when (it.contentTypeOverride) {
                "TEXT" -> 1
                "AUDIO" -> 2
                "IMAGE" -> 3
                "MUSIC" -> 4
            ...
            binding.spContentTypeOverride.setSelection(overrideIndex)
        }
        // 基本信息
...
//        }
    ...
    private fun getSource(): BookSource {
        ...
        // 读取内容类型覆写选择
        source.contentTypeOverride = when (binding.spContentTypeOverride.selectedItemPosition) {
            1 -> "TEXT"
            2 -> "AUDIO"
            3 -> "IMAGE"
            4 -> "MUSIC"
...
//        source.ruleReview = reviewRule
        // 写入初判内容类型与置信度（不覆盖手动覆写）
        if (source.contentTypeOverride.isNullOrBlank()) {
            val (hint, confidence) = ContentTypeDetector.detectSourceTypeHintConfidence(source)
            source.contentTypeHint = hint?.name
            source.contentTypeConfidence = confidence.takeIf { it > 0f }
        }
        return source
    }
