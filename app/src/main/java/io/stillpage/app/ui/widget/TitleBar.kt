...
//            }
    ...
    fun setColorFilter(@ColorInt color: Int) {
        ...
        // 保留原有的 colorFilter 设置，兼容旧实现
        toolbar.children.firstOrNull { it is ImageView }?.background?.colorFilter = colorFilter
        toolbar.navigationIcon?.colorFilter = colorFilter
        toolbar.overflowIcon?.colorFilter = colorFilter
        toolbar.menu.children.forEach { it.icon?.colorFilter = colorFilter }

        // 使用 Drawable tint 强制覆盖导航/菜单图标颜色，避免 XML 中的白色 tint 覆盖
        toolbar.navigationIcon = toolbar.navigationIcon?.mutate()?.apply { setTint(color) }
        toolbar.overflowIcon = toolbar.overflowIcon?.mutate()?.apply { setTint(color) }
        toolbar.menu.children.forEach { item ->
