...
import io.stillpage.app.data.entities.rule.ReviewRule
...
data class BookSource(
    ...
    // 内容类型提示（系统智能判断结果，例如 TEXT/AUDIO/IMAGE/MUSIC/DRAMA/FILE）
    @ColumnInfo(defaultValue = "NULL")
    var contentTypeHint: String? = null,
    // 内容类型置信度（0.0~1.0），用于动态纠偏时的比较
    @ColumnInfo(defaultValue = "NULL")
    var contentTypeConfidence: Float? = null,
    // 手动覆写的内容类型（用户选择后优先级最高）
    @ColumnInfo(defaultValue = "NULL")
    var contentTypeOverride: String? = null
...
//    }
    ...
    fun equal(source: BookSource): Boolean {
        return equal(bookSourceName, source.bookSourceName)
                ...
                && equal(contentTypeHint, source.contentTypeHint)
                && equal(contentTypeOverride, source.contentTypeOverride)
                && contentTypeConfidence == source.contentTypeConfidence
