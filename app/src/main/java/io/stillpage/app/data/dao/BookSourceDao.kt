...
interface BookSourceDao {
     ...
     * 按内容类型筛选书源
     */
    fun flowContentTypeSearch(contentType: String): Flow<List<BookSourcePart>> {
        return when (contentType) {
            "MANUAL" -> {
                // 已手动标记的书源
                flowAll().map { list ->
                    list.filter { part ->
                        part.getBookSource()?.contentTypeOverride != null
                    }
                }
            }
            "AUTO" -> {
                ...
                flowAll().map { list ->
                    list.filter { part ->
                        ...
                        if (bookSource == null) return@filter false
                        val resolvedType = io.stillpage.app.help.ContentTypeResolver.resolveFromSource(bookSource)
                        resolvedType.name == contentType
                    }
                }
            }
        }.flowOn(IO)
